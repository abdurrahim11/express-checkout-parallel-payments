...
$SECFields = array(
					...
					'returnurl' =>  'http://localhost/express-checkout-parallel-payments/express-checkout/GetExpressCheckoutDetails.php', 							    // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
					'cancelurl' =>  'http://localhost/express-checkout-parallel-payments/express-checkout/', 							    // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
...
$Payment = array(
    'amt' => $_SESSION['items'][2]['price'],    // Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
    'currencycode' => 'USD',
    'itemamt' => $_SESSION['items'][2]['price'],       // Subtotal of items only.                                            // A three-character currency code.  Default is USD.
    'shippingamt' => 0,     // Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
    'handlingamt' => 0,     // Total handling costs for this order.  If you specify HANDLINGAMT you mut also specify a value for ITEMAMT.
    'taxamt' => 0,          // Required if you specify itemized L_TAXAMT fields.  Sum of all tax items in this order.
    'paymentaction' => 'Sale',
    'sellerpaypalaccountid' => $_SESSION['items'][2]['seller_id'],         // A unique identifier for the merchant.  For parallel payments, this field is required and must contain the Payer ID or the email address of the merchant.
